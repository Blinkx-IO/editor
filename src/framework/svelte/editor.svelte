<script lang="ts">
	...
	import { configureEditor, contentStatus } from "$visualeditor/editor";
	...
	onMount(() => {
		(async () => {
			...
			editor.on("component:selected", () => {
				selectedComponent.set(editor.getSelected());
			});
			...
			/*editor.Commands.extend('show-component-css',{
				run(){
					showComponentCodeEditor = true;
				},
				stop(){
					showComponentCodeEditor = false;
				}
			})*/
...
<section id="mainEditorSection" bind:this={sectionWrap}>
	<div data-load="initial" id="editor-wrapper" class="overflow-hidden">
		...
		>
			...
			>
				...
				>
					<Status bind:itemId={data.item.id} />
				</div>
			</div>
		</div>
		<div
	...
	<div style="display:none;" id="table-data-container">
		<!--Abdi to place Table Svelte Component here-->
		<DataTableLayout />
	</div>
	{#if showCodeEditor}
		<div
