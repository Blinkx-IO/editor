...
import { EditorTheme } from "@/global";
...
// Helper function to apply theme to all panels
const applyThemeToPanels = (theme: EditorTheme, preference: themePreference) => {
	// Apply theme to SVG elements in panels
	const applyThemeToButtons = (buttons: readonly any[]) => {
		buttons.forEach(button => {
			if (button.label && typeof button.label === 'string' && button.label.includes('svg')) {
				// Add theme-specific classes to SVG elements
				button.label = button.label.replace(
					'class="',
					`class="${preference === 'dark' ? theme.primaryTextColor.dark : theme.primaryTextColor.light} `
				);
			}
		});
	};

	// Apply to all panels with buttons
	if (undoManagerPanel.buttons) applyThemeToButtons(undoManagerPanel.buttons);
	if (devicePanel.buttons) applyThemeToButtons(devicePanel.buttons);
	if (leftPanelGroup.buttons) applyThemeToButtons(leftPanelGroup.buttons);
	if (editAction.buttons) applyThemeToButtons(editAction.buttons);
	if (rightPanelGroup.buttons) applyThemeToButtons(rightPanelGroup.buttons);
	if (utilitiesPanel.buttons) applyThemeToButtons(utilitiesPanel.buttons);
};
